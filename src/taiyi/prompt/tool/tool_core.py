...
TayiArtifact： 工具聚合, 注册, 执行接口
TayiArtifactContext : 工具消息上下文， 恢复的时候 从中恢复 或 保存 

ToolPerMission: allow, ask , deny
ToolPerMissionChecker: 基类， 有若干策略，启动时候的可以从中加载若干的检查策略,  同样需要TayiArtifactContext里的内容
"""
...
#        tool_context: TaiyiArtifactContext
#    ) -> PermissionResult:
#        """执行默认权限检查"""
...
class TaiyiArtifactContext(BaseModel):
