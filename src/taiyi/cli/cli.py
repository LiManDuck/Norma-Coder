...
# Taiyi核心模块
from taiyi.agent.chat_agent import ChatAgent
from taiyi.core.agent_types import (
    BaseAgent
)
from taiyi.agent.taiyi_coder import TaiyiCoder
from taiyi.core.openai_llm import OpenAILLM
from taiyi.cli.repl.repl import TaiyiREPL
import os


class TaiyiCLI:
    """Taiyi AI Agent 命令行界面的主控制器"""
    ...
    def load_config(self) -> dict:
        ...
        """
        # 配置文件路径
        config_path = Path.home() / ".taiyi" / "config.json"
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="taiyi",
        description="太乙真人 - Taiyi AI Agent 智能命令行助手"
    )
    ...
    parser.add_argument("--version", action="version", version="Taiyi AI Agent v0.1.0")
...
def main():
    """命令行入口点"""
    parser = argparse.ArgumentParser(
        description='Taiyi AI Agent - 智能编码助手',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  taiyi                    # 启动交互式会话
  taiyi --config custom    # 使用自定义配置
        """
    )
