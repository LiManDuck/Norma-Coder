...
from taiyi.memory.agent_memory import (
    AgentMemory
)
from taiyi.prompt.system_prompt import SystemPromptService
from taiyi.tool.tool_core import (
    TaiyiArtifact
)

from taiyi.tool import (
    ReadTool,
    LsTool,
    GlobTool,
    GrepTool,
...
class TaiyiCoder(BaseAgent):
    """
    TaiyiCoder - 基于LLM的代码助手Agent
    ...
    def __init__(self,
        ...
        self.tool_manager = TaiyiArtifact(tools=all_tools)
    ...
    async def run(self, query: str) -> AsyncGenerator[Union[AgentEvent, AgentResponse], None]:
        ...
        try:
            ...
            # 6. 错误处理
            # ========================================
            logger.error(f"Error in TaiyiCoder.run: {e}", exc_info=True)
