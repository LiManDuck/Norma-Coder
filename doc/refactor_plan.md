...
- [x] 修复 `TaskStore._save` 非原子写：原直接 ``"w"``+``json.dump`` 全量重写，写入中途崩溃/中断留截断文件、``_load`` 对损坏文件静默返回 [] -> 整个对话任务列表丢失。改为 ``tempfile.mkstemp``+``os.fsync``+``os.replace`` 原子替换（与 WriteTool/EditTool 一致），失败时清理临时文件、原文件完整。新增 `test_tasks.py`（2 项 revert-verified）：CRUD 往返+跨实例持久化+blocks/blockedBy 镜像与清理、原子写失败（patch os.replace 抛错）原文件完整且无残留临时文件。审计 `mcp/tool.py` 无缺陷（不改）。
- [x] 修复 `AgentResponse` 死 validator：stdlib ``@dataclass`` 挂了 pydantic ``@model_validator`` -> validator 永不被调用，``response``/``tool_call_nums`` 自动填充是死代码（构造传 None 就保持 None -> 前端可能空回复）。非当前活跃 bug（norma_coder 三处构造点显式赋值），但具误导性。改为 stdlib ``__post_init__`` 使自动填充真正生效（已显式赋值字段不受影响），清理未用 import。新增 `core/test_agent_types.py`（4 项 revert-verified）。审计 `agent/runner.py`、`core/tool_types.py` 无缺陷（不改）。
- [x] 修复长对话压缩丢失原始请求：``_do_compact`` 原 ``history_text[-8000:]`` 仅取末尾窗口，对话超 8000 字符时丢弃开头的原始用户请求与早期工作 -> 摘要忘记最初目标（codeagent 长任务功能退化）。改为「前 2000 + 末 6000」双窗口（中段省略），短历史原样透传。`test_compact_resume.py` 新增 ``test_compact_preserves_original_request``（6->7 项，revert-verified，``_CapturingLLM`` 断言原始请求在摘要输入中）。审计 `core/llm_types.py`、`cli/command/builtin.py` 无缺陷（不改）。**全部源模块已逐一审计完毕**。
- [x] 移除死代码 todo_tool + 修正系统提示引用不存在的工具：``todo_tool/TodoWriteTool`` 从未注册进 ``default_tools``（实际任务系统是 ``tasks.py``+``task_tool``），是遗留死代码；更糟的是系统提示仍「非常频繁地」指示 LLM 使用根本不存在的 ``TodoWrite``/``TodoRead``。删除整个 ``todo_tool/`` 模块（~340 行）、清理 ``tool/__init__.py`` 导出与 ``permission.py`` 的 ``READ_ONLY_TOOLS`` 条目、系统提示「任务管理」段 5 处改为实际可用的 ``TaskCreate``/``TaskList``/``TaskGet``/``TaskUpdate``、``bash_tool.py`` 4 处子代理提示简化。顺带修 ``llm_types.py`` 形参拼写 ``llm_rquest``->``llm_request`` 与 ``tool_types.py`` 裸 ``except:``->``except Exception:``。回归 22/22 绿，真实 TTY 启动验证通过。净 -345 行。
...
- [x] MCPClient tools/list_changed 通知处理死锁（瘫痪连接 60s）：`_read_loop` 是唯一读 stdout 的协程，原实现收到 `notifications/tools/list_changed` 后原地 `await discover_tools()`（内部又 await 响应），而响应只能由 `_read_loop` 回到 `readline()` 才能读出 -> 互相等待死锁至 `_send_request` 60s 超时；期间该 client 上所有后续请求响应均无法结算，整条连接瘫痪 60s。改 `asyncio.create_task(self._rediscover_tools())` 非阻塞调度 + `_rediscover_tools` 助手封装异常捕获；附带修 `manager.connect_all` 失败路径补 `disconnect` 防孤儿进程。回归 `test_mcp_stdio.py` 新增 `_run_list_changed`（2->3 项，stateful mock 推送 list_changed + 二次 tools/list 返回扩展工具集，8s 内断言重发现出 extra，revert-verify 改 await 即死锁 FAIL 有齿）。
